"use client";

import { motion } from "framer-motion";
import { SpotlightCard, ShinyText } from "@/components/react-bits";

const steps = [
  {
    step: "01",
    title: "Fund treasury",
    description:
      "Connect Sphere wallet on testnet2. UCT balances sync in real time via Sphere Connect.",
  },
  {
    step: "02",
    title: "Issue mandate",
    description:
      "Define reserves, velocity limits, and approval rules through the treasury agent interface.",
  },
  {
    step: "03",
    title: "Policy enforcement",
    description:
      "The engine validates every payment against compiled rules before any settlement attempt.",
  },
  {
    step: "04",
    title: "Autonomous settlement",
    description:
      "Approved transfers execute through Sphere SDK with full activity attribution.",
  },
];

export function HowItWorks() {
  return (
    <section className="px-4 py-16 sm:px-6 sm:py-28" id="how-it-works">
      <div className="mx-auto max-w-3xl">
        <div className="text-center">
          <ShinyText
            text="Workflow"
            className="text-[11px] font-medium uppercase tracking-[0.28em]"
            speed={4}
            color="rgba(232,163,23,0.75)"
            shineColor="#f5d061"
          />
          <h2 className="mt-3 font-display text-2xl font-semibold tracking-tight sm:text-3xl">
            Four steps to autonomous treasury
          </h2>
        </div>

        <div className="relative mt-12 space-y-4 sm:mt-16">
          <div className="absolute left-[23px] top-4 bottom-4 w-px bg-gradient-to-b from-primary/40 via-white/10 to-transparent sm:left-[27px]" />
          {steps.map((item, i) => (
            <motion.div
              key={item.step}
              initial={{ opacity: 0, x: -16 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true }}
              transition={{ delay: i * 0.08 }}
              className="relative flex gap-4 sm:gap-6"
            >
              <div className="relative z-[1] flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border border-primary/25 bg-primary/[0.08] font-mono text-xs font-semibold text-primary shadow-[0_0_20px_rgba(232,163,23,0.12)]">
                {item.step}
              </div>
              <SpotlightCard
                className="depth-panel min-w-0 flex-1 rounded-2xl border border-white/[0.06] p-4 sm:p-5"
                spotlightColor="rgba(232, 163, 23, 0.14)"
              >
                <h3 className="font-display text-base font-semibold sm:text-lg">
                  {item.title}
                </h3>
                <p className="mt-1.5 text-sm leading-relaxed text-muted-foreground">
                  {item.description}
                </p>
              </SpotlightCard>
            </motion.div>
          ))}
        </div>
      </div>
    </section>
  );
}
